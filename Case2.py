...
with tab2:
    ...
    # Voeg een trendlijn toe met behulp van 'statsmodels'
    X = sm.add_constant(df[selected_feature])  # Voeg een constante term toe aan de regressie (intercept)
    model = sm.OLS(data['price'], X).fit()  # Lineaire regressie uitvoeren

    # Bereken R-squared (R²)
    r_squared = model.rsquared

    # Maak een tabel met R-squared
